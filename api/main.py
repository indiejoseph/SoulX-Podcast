...
async def generate_sync(
    ...
    prompt_texts: str = Form(..., description='Reference transcripts as a JSON array, e.g. ["text 1", "text 2"].'),
...
async def generate_stream(
    ...
    top_p: float = Form(default=0.9, ge=0.0, le=1.0, description="Top-p sampling parameter."),
    repetition_penalty: float = Form(default=1.25, ge=1.0, le=2.0, description="Repetition penalty."),
    output_format: str = Form(default="wav", description="Output format: wav or pcm."),
    chunk_size: Optional[int] = Form(default=None, ge=1, le=500, description="Speech token chunk size for streaming. Defaults to server STREAM_CHUNK_SIZE."),
    first_chunk_size: Optional[int] = Form(default=None, ge=1, le=100, description="First chunk size for low TTFA. Defaults to server STREAM_FIRST_CHUNK_SIZE."),
    _: None = Depends(require_api_key),
):
    """
    Stream speech synthesis with chunked transfer encoding.

    Returns audio as a chunked WAV (or raw PCM) stream. The first bytes arrive
    after STREAM_FIRST_CHUNK_SIZE speech tokens are generated by the LLM
    (~160 ms at 25 Hz with the default value of 4), giving much lower TTFA
    than the non-streaming endpoints.

    Use `output_format=pcm` to skip the WAV header and receive raw PCM16LE
    at 24 kHz mono — useful when the client handles framing itself.
    """
    if output_format not in ("wav", "pcm"):
        raise HTTPException(status_code=400, detail="output_format must be 'wav' or 'pcm'")
    ...
    try:
        ...
        logger.info("Stream generation started: task_id=%s, speakers=%d", task_id, len(audio_paths))

        return StreamingResponse(
            service.stream_speech_podcast(
                prompt_audio_paths=audio_paths,
                prompt_texts=prompt_text_list,
                dialogue_text=dialogue_text,
                seed=seed,
